	...
	return (
		<div className="flex flex-col items-center justify-center h-1/2 p-8 bg-gray-100">
			...
			{PartAttendance.map(part => (
				...
				>
					<p className="p-2 m-2">
						<strong>Date:</strong> <span className="profileStat">{part.date}</span>
					</p>
					<p className="p-2 m-2">
						<strong>Course:</strong>{' '}
						<span className="profileStat">{part.course}</span>
					</p>
					<p className="p-2 m-2">
						<strong>Part:</strong> <span className="profileStat">{part.part}</span>
					</p>
					<p className="p-2 m-2">
						<strong>Length:</strong>{' '}
						<span className="profileStat">{part.length}h</span>
					</p>
					<p className="p-2 m-2">
						<strong>Attendance:</strong>
						<button
							onClick={() => toggleDropdown(part.id)}
							className={`profileStat ${getAttendanceColorClass(
								part.attendance,
					...
					{openDropdown === part.id && (
						<div className="absolute right-0 mt-2 p-1 bg-white border border-gray-300 rounded shadow-md z-10">
							<p
								className="cursor-pointer hover:bg-metropoliaTrendGreen"
								onClick={() => handleOptionClick('Present', part.id)}
							>
								Present
							</p>
							<p
								className="cursor-pointer hover:bg-metropoliaSupportRed"
								onClick={() => handleOptionClick('Not Present', part.id)}
							>
								Not Present
							</p>
